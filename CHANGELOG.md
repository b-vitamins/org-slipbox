...
- Wrap glossary headwords and their controls when the definition pane is narrow,
  keeping Open in reader reachable even beside a long title.
- Returning from a narrow glossary definition clears its deep link, so changing
  the listing does not reopen the previous definition or hide the term list.
- Each narrow reading-trail landmark names its note position, so assistive
  technology can distinguish the navigation controls of adjacent columns.
- Provision Emacs in the benchmark CI job, which runs the buffer benchmarks.
- Require Vitest 4.1.11, patched against GHSA-82fw-gwwq-j7x9.
- Let jsdom supply browser storage in DOM tests on Node 25 and later, where
  Node's file-backed storage otherwise masks it.
- Release metadata checks distinguish an unreleased candidate from a shipped
  version; tagged builds require the latter.
- Reject missing and whitespace-only changelog sections when extracting
  GitHub release notes.
- Write archive-relative paths in the release checksum manifest, so verification
  works directly from the downloaded asset directory.
...
- Sample incremental file indexing 40 times in both benchmark profiles, giving
  p95 a measurable tail while retaining the latency budgets and raw maximum.
- Isolate hosted CI benchmarks from shared-disk latency with memory-backed
  scratch storage and retained raw measurements; keep local release benchmarks
  on the host filesystem and leave all latency budgets unchanged.
