...
test.describe("accessibility audit", () => {
  ...
  test("covers every desktop surface", async ({ page }) => {
    ...
    await page.goto("/?note=file:alpha.org&stacked=file:beta.org");
    await expect(page.getByRole("heading", { name: "Beta" })).toBeVisible();
    await expect(page.locator(".spine-column")).toHaveCount(2);
    await expectAccessible(page, "two-column reader");
...
test.describe("mobile accessibility audit", () => {
  ...
  test("covers entry, glossary, reader, and error states", async ({ page }) => {
    ...
    await expect(page.getByRole("listbox", { name: "Glossary terms" })).toBeVisible();
    expect(new URL(page.url()).searchParams.has("term")).toBe(false);
    await page.getByRole("button", { name: "Due terms" }).tap();
    await expect(page.getByText(/^1 term is due,/)).toBeVisible();
    await expectAccessible(page, "mobile glossary due list");

    await page.getByRole("option", { name: "Alpha" }).tap();
    await expect(page.getByRole("button", { name: "Back to terms" })).toBeVisible();
    await expectAccessible(page, "mobile glossary due detail");
    ...
    await expect(page.getByRole("heading", { name: "Beta" })).toBeVisible();
    await expect(page.locator(".spine-column")).toHaveCount(2);
    await expectAccessible(page, "mobile reading trail");
