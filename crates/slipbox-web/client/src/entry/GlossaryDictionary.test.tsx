...
describe("GlossaryDictionary", () => {
  ...
  it("returns to the list without reopening a definition when the listing changes", async () => {
    vi.stubGlobal("fetch", pagedFetch(twoListWorld));
    const termUrl = memoryTermUrl("notes/b.org::0");

    mount({ termUrl });
    await screen.findByRole("option", { name: "Beta" });
    expect(document.querySelector(".glossary")).toHaveClass(
      "glossary--detail-open",
    );

    fireEvent.click(screen.getByRole("button", { name: "Back to terms" }));

    expect(termUrl.read()).toBeNull();
    expect(screen.getByRole("option", { name: "Beta" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
    expect(document.querySelector(".glossary")).not.toHaveClass(
      "glossary--detail-open",
    );
