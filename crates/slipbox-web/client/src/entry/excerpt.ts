...
/**
 * The runs of prose a content-search excerpt draws as, in order. Unbounded: the
 * server abridges the excerpt and the surface clips it by line box, not by
 * character count.
 */
export function excerptRuns(
  segments: readonly ContentSegment[],
): ExcerptRun[] {
  const source = dropCutTail(dropCutHead(spliceMarkers(segments)));
