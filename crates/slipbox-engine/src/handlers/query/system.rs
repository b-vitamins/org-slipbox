...
use crate::rpc::{internal_error, parse_params, path_denied, scan_error, to_value};
...
pub(crate) fn index(state: &mut ServerState) -> Result<serde_json::Value, JsonRpcError> {
    let files =
        slipbox_index::scan_root_with_platform(&state.root, &state.discovery, &state.platform)
            .map_err(|error| scan_error(error, &state.root, "failed to scan Org files"))?;
    let stats = state
        .database
        .sync_index(&files)
