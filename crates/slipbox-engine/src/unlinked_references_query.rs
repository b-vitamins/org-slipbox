...
use slipbox_index::PlatformPolicy;
...
///
/// `platform` authorizes reading the queried node's own source; an unauthorized
/// capability fails the query.
pub fn query_unlinked_references(
    database: &Database,
    root: &Path,
    node: &AnchorRecord,
    limit: usize,
    ...
    for file_path in indexed_files {
        ...
        let current_range = if file_path == node.file_path {
            Some(current_subtree_range(root, &file_path, node, platform)?)
        } else {
            None
        };
...
fn current_subtree_range(
    ...
    let source = slipbox_index::read_source_with_platform(&absolute_path, platform)
