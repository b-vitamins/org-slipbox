...
Hosted Linux CI uses memory-backed scratch storage to isolate application
regressions from shared-disk latency. Local benchmark commands, including the
release gate, use the host filesystem. CI retains the full timing samples,
maximums, and scratch-filesystem information as a workflow artifact; its write
budgets are not physical-disk latency guarantees.
